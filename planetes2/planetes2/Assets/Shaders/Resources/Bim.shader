Shader "Vertex/Bim" {
	...
	void geom(triangle GS_INPUT tri[3], inout TriangleStream<FS_INPUT> triStream)
	{
		...
		float angle = center.y * _Slider;

		// float3 offset = float3(t, t * 3.0, 0);
		// float dist = noiseIQ((tri[0].normal + offset) * 2.0);
		// dist *= 0.05;

		float offset = noiseIQ(rotateY(triNormal * 10.0,t));

		float distRatio = fmod(t + offset, 1.0);
		distRatio = smoothstep(0.5, 1.0, distRatio);
